...
public class RestSolicitudController {
	...
	////1
	@RequestMapping(value="/solicitudes/{id}", method = RequestMethod.GET , produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	...
	public ResponseEntity<ResultMessage> listadodeSolicitudesxPersona(@PathVariable("id") Long personaId, Locale objLocale) throws Exception{	
	...
	//	response.setData(solicitudService.obtenerSolicitudes(pSolicitudFiltroBean));
		response.setData(solicitudService.listadodeSolicitudesxPersona(personaId));
